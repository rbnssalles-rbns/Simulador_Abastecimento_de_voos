...
#Entrada de voos (Excel ou Imagem OCR)
    ...
    if df_voos is not None and not df_voos.empty:
        ...
        # Regras de refeições por classe
        regras = {
            "Classe A": ["Refeição Executiva"],
            "Classe B": ["Refeição Executiva", "Lanche"],
            "Classe C": ["Lanche"]
        }
        ...
        for _, voo in df_voos.iterrows():
            ...
            # Calcula consumo por tipo de refeição e item
            for classe, tipos_refeicao in regras.items():
                qtd_pax = passageiros[classe]
                for tipo in tipos_refeicao:
                    itens_refeicao = df_refeicoes[df_refeicoes["Tipo de Refeição"] == tipo]
                    for _, item in itens_refeicao.iterrows():
                        consumo_voo.append({
                            "ID Item": item["ID Item"],
                            "Item": item["Item"],
                            "Quantidade": item["Quantidade"] * qtd_pax,
                            "Voo": voo["Nº do Voo"]
                        })

            # Agrupa consumo por item
            df_consumo_voo = pd.DataFrame(consumo_voo).groupby(["ID Item", "Item", "Voo"]).sum().reset_index()

            # Mapeia unidades
            map_unidades = dict(zip(st.session_state.estoque["ID"], st.session_state.estoque["Unidade Base"]))

            # Normaliza unidades e calcula embalagens
            df_consumo_voo[["Quantidade Normalizada", "Unidade", "Unidades Embalagem"]] = df_consumo_voo.apply(
                lambda row: pd.Series(normalizar_consumo(
                    row["Item"],
                    row["Quantidade"],
                    map_unidades.get(row["ID Item"], "un"),
                    embalagem_ml=350 if row["Item"].lower() == "refrigerante" else None
                )),
                axis=1
            )
            ...
            for _, row in df_consumo_voo.iterrows():
                if row["ID Item"] in st.session_state.estoque["ID"].values:
                    st.session_state.estoque.loc[
