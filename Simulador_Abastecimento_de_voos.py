...
if uploaded_file:
    ...
    if uploaded_reposicao:
        ...
        # Captura o saldo antes da reposição
        estoque_antes = st.session_state.estoque.copy()
        ...
        # Captura o saldo depois da reposição
        estoque_depois = st.session_state.estoque.copy()

        # Monta relatório de impacto da reposição
        relatorio_reposicao = []
        for _, row in df_reposicao.iterrows():
            item_id = row["ID"]
            item_nome = row["Item"]
            ...
            saldo_anterior = estoque_antes.loc[estoque_antes["ID"] == item_id, "Estoque inicial"].values[0]                 if item_id in estoque_antes["ID"].values else 0
            saldo_novo = estoque_depois.loc[estoque_depois["ID"] == item_id, "Estoque inicial"].values[0]
