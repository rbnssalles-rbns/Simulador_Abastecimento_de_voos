...
if uploaded_reposicao:
    ...
    # Captura o saldo anterior a partir do último registro consolidado
    saldo_anterior_por_id = {}
    if "historico_voos" in st.session_state and st.session_state.historico_voos:
        for registro in reversed(st.session_state.historico_voos):
            df_saldo = registro["saldo atualizado"]
            for _, row in df_saldo.iterrows():
                item_id = row["ID"]
                if item_id not in saldo_anterior_por_id:
                    saldo_anterior_por_id[item_id] = row["Estoque inicial"]

    # Aplica reposição e monta relatório
    relatorio_reposicao = []
    ...
    for _, row in df_reposicao.iterrows():
        ...
        saldo_anterior = saldo_anterior_por_id.get(item_id)
        if saldo_anterior is None:
            saldo_anterior = st.session_state.estoque.loc[
                st.session_state.estoque["ID"] == item_id, "Estoque inicial"
            ].values[0] - qtd_reposta

        saldo_novo = st.session_state.estoque.loc[
            st.session_state.estoque["ID"] == item_id, "Estoque inicial"
        ].values[0]

        relatorio_reposicao.append({
            "ID": item_id,
            "Item": item_nome,
            "Saldo Anterior": saldo_anterior,
            "Quantidade Reposta": qtd_reposta,
            "Novo Saldo": saldo_novo
        })

    df_relatorio_reposicao = pd.DataFrame(relatorio_reposicao)

    st.success("✅ Estoque atualizado com a reposição.")
    st.subheader("📦 Estoque Atualizado")
    st.dataframe(st.session_state.estoque)

    st.subheader("📊 Impacto da Reposição")
    st.dataframe(df_relatorio_reposicao)
