...
# Padroniza nomes de colunas do estoque e refeições
...
# Inicializa cache apenas se estoque já existir
if "estoque" in st.session_state:
        ...
        for _, voo in df_voos.iterrows():
            ...
            for classe, qtd_pax in passageiros.items():
                ...
                for _, item in df_refeicoes.iterrows():
                    coluna_quantidade = f"quantidade {classe.split()[-1]}"
    ...
    st.error("❌ Estoque não foi carregado. Verifique a aba 'Cadastro de Itens' antes de rodar a simulação.")
