...
if uploaded_reposicao:
    ...
    df_reposicao.columns = df_reposicao.columns.str.strip().str.lower()
    ...
    for _, row in df_reposicao.iterrows():
        ...
        unidade = str(row.get("unidade base")).lower()
        qtd_reposta = row.get("quantidade reposta")
...
if "estoque" in st.session_state and df_refeicoes is not None:
    # Padroniza colunas
    st.session_state.estoque.columns = st.session_state.estoque.columns.str.strip().str.lower()
    df_refeicoes.columns = df_refeicoes.columns.str.strip().str.lower()
    if df_voos is not None:
        df_voos.columns = df_voos.columns.str.strip().str.lower()

    # Garante que os campos de quantidade sejam numéricos
    for col in ["quantidade classe a", "quantidade classe b", "quantidade classe c"]:
        if col in df_refeicoes.columns:
            df_refeicoes[col] = pd.to_numeric(df_refeicoes[col], errors="coerce")
        ...
        for _, voo in df_voos.iterrows():
            ...
            for classe, qtd_pax in passageiros.items():
                ...
                for _, item in df_refeicoes.iterrows():
                    coluna_quantidade = f"quantidade classe {classe}"
            ...
            for _, row in df_consumo_voo.iterrows():
                if row["id item"] in estoque["id"].values:
                    # Verifica se a coluna correta de unidade existe
                    if "unidade base" in estoque.columns:
                        unidade_base = estoque.loc[estoque["id"] == row["id item"], "unidade base"].values[0]
                    elif "unidade" in estoque.columns:
                        unidade_base = estoque.loc[estoque["id"] == row["id item"], "unidade"].values[0]
                    else:
                        unidade_base = None
        ...
        for registro in st.session_state.historico_voos:
            for _, row in registro["consumo"].iterrows():
                unidade_item = st.session_state.estoque_cache.loc[
                    st.session_state.estoque_cache["id"] == row["id item"], "unidade base"
